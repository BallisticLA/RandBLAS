...
class TestLSKGE3 : public ::testing::Test
{
    ...
    static void test_sketch_vec_wide(
        ...
        int64_t d,     // Dim of y_wide and y_ref
        ...
        // Initialize elemnts of x to 1 with stride of incx between elements
        ...
        // Perform reference sketch using GEMV
    ...
    template<typename T>
    static void test_sketch_vec_tallvswide(
        uint32_t seed, // Seed for S_wide
        int64_t d,     // Dim of y
        int64_t m,     // Dim of x. Expect m > d.
        int64_t incx,  // Stride between elements of x
        ...
        RandBLAS::DenseDist D_tall(m, d);
        RandBLAS::DenseSkOp<double> S_wide(D_wide, seed, nullptr);
        RandBLAS::fill_dense(S_wide);
        RandBLAS::DenseSkOp<double> S_tall(D_tall, seed, nullptr);
        RandBLAS::fill_dense(S_tall);
        ...
        // Compare entrywise results of sketching with sketch_vector and using gemv
        for (int i = 0; i < d; i++) {
            total += abs(y_wide[incy*i] - y_tall[incy*i]);
        }
    ...
    static void test_sketch_vec_tallSK(
        ...
        // Generate tall sketching operator
        RandBLAS::DenseDist D_tall(d, m);
        RandBLAS::DenseSkOp<double> S_tall(D_tall, seed, nullptr);
        RandBLAS::fill_dense(S_tall);

        // Perform tall sketch
        RandBLAS::sketch_vector<T>(blas::Op::NoTrans, d, m, 1, S_tall, 0, 0, x, incx, 0, y_tall, incy);
        
        // Perform reference sketch using GEMV
        int64_t lds = (S_tall.layout == blas::Layout::RowMajor) ? m : d;
        blas::gemv(S_tall.layout, blas::Op::NoTrans, d, m, 1, S_tall.buff, lds, x, incx, 0, y_ref, incy); 
...
//
////////////////////////////////////////////////////////////////////////
TEST_F(TestLSKGE3, test_sketch_vec_tallSK)
{
...
TEST_F(TestLSKGE3, test_sketch_vec_tallvswide)
{
    for (uint32_t seed : {0, 1, 2}) {
        test_sketch_vec_tallvswide<double>(seed, 100, 1000, 2, 3);
        test_sketch_vec_tallvswide<double>(seed, 101, 1013, 3, 2);
    }
}
...
{
    for (uint32_t seed : {0, 1, 2}) {
        test_sketch_vec_tall<double>(seed, 100, 1000, 2, 3);
        test_sketch_vec_tall<double>(seed, 101, 1013, 3, 2);
    }
}
