...
class TestSubmatGeneration : public ::testing::Test
{
    ...
    static void test_colwise_smat_gen(
        ...
        double* smat = new double[n_srows*n_scols];
        RandBLAS::dense::fill_rmat<T,RNG,OP>(n_rows, n_cols, mat, seed);
        int ind = 0; //variable used for indexing smat when comparing to rmat
        double total = 0; //total holds the total differences of the entries of the submatrix generated by fill_rmat and fill_rsubmat_omp
        for (int nptr = ptr; nptr < n_cols*(n_rows-n_srows-1); nptr += n_cols) { //Loop through various pointer locations.- goes down the random matrix
            ...
            for (int i = 0; i<n_srows; i++) { //Loop through entries of the submatrix
                for (int j = 0; j<n_scols; j++) {
                    total += abs(smat[ind] - mat[nptr + i*n_cols + j]); //Compare absolute value of the entrywise difference and add it to total
                    ind++;
                }
            }
        }
        delete[] mat;
        delete[] smat;
        EXPECT_EQ(total, 0); //Submatrices are equal if each entry is bitwise the same
    }
    ...
    static void test_rowwise_smat_gen(
        ...
        for (int nptr = ptr; nptr < (n_cols - n_scols - 1); nptr += 1) {
            ...
            for (int i = 0; i<n_srows; i++) { //Loop through entries of the submatrix
                for (int j = 0; j<n_scols; j++) {
                    total += abs(smat[ind] - mat[nptr + i*n_cols + j]); //Compare entries of submatrices (Expected to be 0)
                    ind++;
                }
            }
        }
    ...
    static void test_diag_smat_gen(
        ...
        double* mat = new double[n_rows*n_cols];
        RandBLAS::dense::fill_rmat<double,r123::Philox4x32,r123ext::boxmul>(n_rows, n_cols, mat, seed);
        int ind = 0;
        double total = 0;
        int64_t n_scols = 1;
        int64_t n_srows = 1;
        for (int ptr = 0; ptr + n_scols + n_cols*n_srows < n_cols*n_rows; ptr += n_rows+1) { // Loop through the diagonal of the matrix
            double smat[n_scols*n_srows];
            RandBLAS::dense::fill_rsubmat_omp<double,r123::Philox4x32,r123ext::boxmul>(n_cols, smat, n_srows, n_scols, ptr, seed);
            ind = 0;
            for (int i = 0; i<n_srows; i++) { //Loop through entries of the submatrix
                for (int j = 0; j<n_scols; j++) {
                    total += abs(smat[ind] - mat[ptr + i*n_cols + j]); //Compare entries of submatrices (Expected to be 0)
                    ind++;
                }
            }
...
TEST_F(TestSubmatGeneration, Gaussian)
{
    RandBLAS::base::RNGState<r123::Philox4x32> seed(1);
    int64_t n_rows = 100;
    int64_t n_cols = 2000;
    int64_t n_srows = 40;
    int64_t n_scols = 100;
    ...
    for (int k = 0; k < 5; k++) {
        ...
        test_colwise_smat_gen<double, r123::Philox4x32,r123ext::boxmul>(n_cols, n_rows, n_scols, n_srows, ptr, seed);
        test_rowwise_smat_gen<double, r123::Philox4x32,r123ext::boxmul>(n_cols, n_rows, n_scols, n_srows, ptr, seed);
        test_diag_smat_gen<double, r123::Philox4x32, r123ext::boxmul>(n_cols, n_rows, seed);
    }
