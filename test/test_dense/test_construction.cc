...
class TestSubmatGeneration : public ::testing::Test
{
    ...
    static void test_colwise_smat_gen(
        ...
        T total = 0; //total holds the total differences of the entries of the submatrix generated by fill_rmat and fill_rsubmat_omp
    ...
    static void test_diag_smat_gen(
        ...
        T* mat = new T[n_rows*n_cols];
        RandBLAS::dense::fill_rmat<T,r123::Philox4x32,r123ext::boxmul>(n_rows, n_cols, mat, seed);
        int ind = 0;
        T total = 0;
        int64_t n_scols = 1;
        int64_t n_srows = 1;
        for (int ptr = 0; ptr + n_scols + n_cols*n_srows < n_cols*n_rows; ptr += n_rows+1) { // Loop through the diagonal of the matrix
            T smat[n_scols*n_srows];
            RandBLAS::dense::fill_rsubmat_omp<T,r123::Philox4x32,r123ext::boxmul>(n_cols, smat, n_srows, n_scols, ptr, seed);
            ind = 0;
            for (int i = 0; i<n_srows; i++) { //Loop through entries of the submatrix
                for (int j = 0; j<n_scols; j++) {
