...
void create_csc_from_coo_one_based(MKL_INT d, MKL_INT m, MKL_INT nnz, MKL_INT *mkrows, 
	MKL_INT *mkcols, int *vals, sparse_matrix_t *output){
	assert(nnz != 0);
	sortArr_csc(mkcols, nnz, mkrows, mkcols, vals); // sort based on col index
	...
	int status = mkl_sparse_d_create_csc(output, SPARSE_INDEX_BASE_ONE, d, m, csr_cols_start, csr_cols_end, csr_rows, dvals);
	/*
	for (int i = 0; i < nnz; i++){
		std::cout << "csr_cols: " << csr_cols[i] << " ";
		std::cout << "csr_vals: " << csr_vals[i] << " ";
...
*/
    ...
    if (c_major) {
		...
		std::cout << duration_cast<milliseconds>(t2 - t1).count() << "ms for csr/column major computation time\n";


		sparse_matrix_t s_csc;
		create_csc_from_coo_one_based(d, m, nnz, rows, cols, vals, &s_csc);
		printf("got here\n");
		double *b_hat = (double*) calloc(d * n, sizeof(double));
		t1 = high_resolution_clock::now();
		mkl_sparse_d_mm(SPARSE_OPERATION_NON_TRANSPOSE, 1.0, s_csc, desc, SPARSE_LAYOUT_COLUMN_MAJOR,
						a, n, m, // n = # cols in output, m = leading dimension of data matrix
						0.0,
						b_hat, d);
		t2 = high_resolution_clock::now();
		std::cout << duration_cast<milliseconds>(t2 - t1).count() << "ms for csc/column major computation time\n";

		for(int i = 0; i < d * n; i++){
			assert(abs(a_hat[i] - b_hat[i]) < 1e-12);
		}
	} else {
		...
		t2 = high_resolution_clock::now();
		std::cout << duration_cast<milliseconds>(t2 - t1).count() << "ms for csr/row major computation time\n";
		...
		mkl_sparse_d_mm(SPARSE_OPERATION_NON_TRANSPOSE, 1.0, s_csc, desc, SPARSE_LAYOUT_ROW_MAJOR,
				...
				a_hat, n); // n = leading dimension of output matrix
		t2 = high_resolution_clock::now();
		std::cout << duration_cast<milliseconds>(t2 - t1).count() << "ms for csc/row major computation time\n";
	...
	#pragma omp parallel default(shared)
	{
		...
		for(int c = 0; c < d; c++){
			...
			for (int r = rows[c]; r < rows[c + 1]; r++){
				int ori_col = cols[r];
                int sign = sgn(ori_col);
                int col = ori_col * sign - 1;
                cblas_daxpy(n, sign, &a[col * n], 1, a_hat_row, 1);
				//cblas_daxpy(n, vals[r], &a[col * n], 1, a_hat_row, 1);
				// for(int i = 0; i < n; i++)
				// {
				// 	a_hat_row[i] += sign * a[col * n + i]; 
				// }
			}
		}
	...
	#pragma omp parallel default(shared)
	{
		...
		for(int outer = 0; outer < max_threads; outer++){
			for(int c = 0; c < m; c++){
				...
				for (int r = 0; r < k; r++){
					int inner = c * k + r;
                    int ori_row = rows[inner];
					int row = ori_row * sgn(ori_row) - 1;
					if(row >= index_range[my_id] && row < index_range[my_id + 1]){
                        cblas_daxpy(n, sgn(ori_row), a_row, 1, &a_hat[row * n], 1);
						//cblas_daxpy(n, vals[inner], a_row, 1, &a_hat[row * n], 1);
					}	
					// if(row == 0)
					// 	cc++;
				}
...
void apply_my_sjlt_csc_and_pretend_col(int d, int m, int n, int nnz, int *rows, int *cols, int *vals, double *a, double *a_hat){
	int k = nnz / m;
	int max_threads = omp_get_max_threads();
	int index_range[max_threads + 1];
	index_range[0] = 0;
		...
		for(int outer = 0; outer < max_threads; outer++){
			for(int c = 0; c < m; c++){
				...
				for (int r = 0; r < k; r++){
					...
					int row = ori_row * sgn(ori_row) - 1;
					if(row >= index_range[my_id] && row < index_range[my_id + 1]){
                        cblas_daxpy(n, sgn(ori_row), a_row, m, &a_hat[row], d);
						//cblas_daxpy(n, vals[inner], a_row, 1, &a_hat[row * n], 1);
					}	
					// if(row == 0)
					// 	cc++;
...
void apply_my_sjlt_csr_and_col(int d, int m, int n, int nnz, int *rows, int *cols, int *vals, double *a, double *a_hat){

	#pragma omp parallel default(shared)
	{
		//double *buffer_vals = (double*) calloc(m, sizeof(double));
		//double *buffer_other = (double*) calloc(m, sizeof(double));
		#pragma omp for schedule(static)
		for(int k = 0; k < n; k++){
			//double *a_hat_col = &a_hat[k * d];
			double *a_col = &a[m * k];
			for(int c = 0; c < d; c++){
				...
				for (int r = left; r < right; r++){
					...
					int sign = sgn(ori_col);
					int col = ori_col * sign - 1;
					//buffer_vals[r - left] = sign;
					//buffer_other[r - left] = a_col[col];
					a_hat[k * d + c] += (sign * a_col[col]);
				}
				//int total_count = right - left;
				//a_hat[k * d + c] = cblas_ddot(total_count, buffer_vals, 1, buffer_other, 1);
...
void apply_my_sjlt_csc_and_col(int d, int m, int n, int nnz, int *rows, int *cols, int *vals, double *a, double *a_hat, int per_col){
	#pragma omp parallel default(shared)
	{
		...
		for(int k = 0; k < n; k++){
			...
			for(int c = 0; c < m; c++){
				...
				for (int r = c * per_col; r < (c + 1) * per_col; r++){
					int ori_row = rows[r];
					int sign = sgn(ori_row);
					int row = ori_row * sign - 1;

					a_hat[k * d + row] += (sign * scale);
...
double * test_gen_and_apply_sjlt_naive(int m, int d, int n, int k) {
    ...
    double *a = (double*) calloc(n * m, sizeof(double));
    random_vector(n * m, a, 0);
	double *a_transpose = (double*) calloc(n * m, sizeof(double));
	for(int r = 0; r < n; r++)
    {
        for(int c = 0; c < m; c++)
        {
            a_transpose[c + r * m] = a[c * n + r];
        }
    }
    ...
    int *cols = (int*) calloc(nnz, sizeof(int));
    int *vals = (int*) calloc(nnz, sizeof(int));
    int *encoded = (int*) calloc(nnz, sizeof(int));
    gen_sjlt(d, m, k, rows, cols, vals, 0);

    for(int i = 0; i < nnz; i++)
    {
        encoded[i] = (rows[i] + 1) * vals[i];
    }
	...
	printf("csc/row parallel my method 1\n");
	double *a_hat = (double*) calloc(d * n, sizeof(double));

    clock_gettime(CLOCK_MONOTONIC, &start);
	apply_my_sjlt_csc(d, m, n, nnz, encoded, cols, vals, a, a_hat);
    clock_gettime(CLOCK_MONOTONIC, &end);
	delta_t = 1e9 * (end.tv_sec - start.tv_sec) + end.tv_nsec - start.tv_nsec;
	delta_t /= 1e9;
	printf("\t%f\n", delta_t);
...
/*
	printf("csc parallel riley method 2\n");
	double *b_hat = (double*) calloc(d * n, sizeof(double));
	clock_gettime(CLOCK_MONOTONIC, &start);
	//add_function();
...
*/
/*
    printf("csc and col major for dense\n");
    double *a_transpose = (double*) calloc(n * m, sizeof(double));
    double *d_hat = (double*) calloc(d * n, sizeof(double));
    double *d_hat_transpose = (double*) calloc(d * n, sizeof(double));
    for(int r = 0; r < n; r++)
    {
        for(int c = 0; c < m; c++)
        {
    ...
    clock_gettime(CLOCK_MONOTONIC, &start);
	apply_my_sjlt_csc_and_pretend_col(d, m, n, nnz, encoded, cols, vals, a_transpose, d_hat_transpose);
    clock_gettime(CLOCK_MONOTONIC, &end);
	delta_t = 1e9 * (end.tv_sec - start.tv_sec) + end.tv_nsec - start.tv_nsec;
	delta_t /= 1e9;
	printf("\t%f\n", delta_t);
    ...
    for(int r = 0; r < d; r++)
    {
        for(int c = 0; c < n; c++)
        {
            d_hat[c + r * n] = d_hat_transpose[r + c * d];
        }
    }
...
*/
	...
	printf("csc/col parallel\n");
	double *f_hat = (double*) calloc(d * n, sizeof(double));
	double *f_hat_transpose = (double*) calloc(d * n, sizeof(double));
    clock_gettime(CLOCK_MONOTONIC, &start);
	apply_my_sjlt_csc_and_col(d, m, n, nnz, encoded, cols, vals, a_transpose, f_hat_transpose, k);
    clock_gettime(CLOCK_MONOTONIC, &end);
	delta_t = 1e9 * (end.tv_sec - start.tv_sec) + end.tv_nsec - start.tv_nsec;
	delta_t /= 1e9;
	printf("\t%f\n", delta_t);
	for(int r = 0; r < d; r++)
    {
        for(int c = 0; c < n; c++)
        {
            f_hat[c + r * n] = f_hat_transpose[r + c * d];
        }
    }
	...
	for(int i = 0; i < nnz; i++)
    {
        encoded[i] = (cols[i] + 1) * vals[i];
    }

	printf("csr/row parallel\n");
	double *e_hat = (double*) calloc(d * n, sizeof(double));
	clock_gettime(CLOCK_MONOTONIC, &start);
	apply_my_sjlt_csr(d, m, n, nnz, row_count, encoded, vals, a, e_hat);
    clock_gettime(CLOCK_MONOTONIC, &end);
	delta_t = 1e9 * (end.tv_sec - start.tv_sec) + end.tv_nsec - start.tv_nsec;
	delta_t /= 1e9;
	printf("\t%f\n", delta_t);
	...
	printf("csr/col parallel\n");
    double *c_hat = (double*) calloc(d * n, sizeof(double));
    double *c_hat_transpose = (double*) calloc(d * n, sizeof(double));
    clock_gettime(CLOCK_MONOTONIC, &start);
	apply_my_sjlt_csr_and_col(d, m, n, nnz, row_count, encoded, vals, a_transpose, c_hat_transpose);
    clock_gettime(CLOCK_MONOTONIC, &end);
	delta_t = 1e9 * (end.tv_sec - start.tv_sec) + end.tv_nsec - start.tv_nsec;
	delta_t /= 1e9;
	printf("\t%f\n", delta_t);
	...
	for(int r = 0; r < d; r++)
    {
        for(int c = 0; c < n; c++)
        {
            c_hat[c + r * n] = c_hat_transpose[r + c * d];
        }
    }
	...
	for(int i = 0; i < d * n; i++){
		...
		//if(abs(a_hat[i] - c_hat[i]) >= 1e-12){
			//printf("difference: %.10e\n", abs(a_hat[i] - c_hat[i]));
		assert(abs(a_hat[i] - c_hat[i]) < 1e-12);
		//}
		
        //assert(abs(c_hat[i] - d_hat[i]) < 1e-13);
		assert(abs(a_hat[i] - e_hat[i]) < 1e-12);
		assert(abs(a_hat[i] - f_hat[i]) < 1e-12);
	}
...
int main() {
	...
	int k = 8;
	int n = 2000;
	std::cout << "size of MKL INT: " << sizeof(MKL_INT) << "\n";
